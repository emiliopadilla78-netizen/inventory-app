  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-extrabold tracking-tight">Sales History</h1>
        <p className="text-muted-foreground text-lg">View past transactions and receipts.</p>
      </div>
                ...
                <TableRow>
                  <TableCell colSpan={4} className="h-48 text-center text-muted-foreground">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-lg font-medium text-foreground">No sales recorded yet</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
          ...
          {selectedSale && (
            <>
              <div className="bg-primary/5 p-6 border-b border-border/50 text-center">
                ...
                <DialogTitle className="text-2xl font-extrabold mb-1">Receipt</DialogTitle>
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-6 pb-6 border-b border-dashed border-border">
                  ...
                  <div className="font-mono font-medium">#{selectedSale.id.toString().padStart(6, '0')}</div>
                </div>
                ...
                <div className="space-y-4 mb-6">
                  {selectedSale.items?.map((item) => (
                    <div key={item.id} className="flex justify-between items-start text-sm">
                      <div>
                        <div className="font-medium text-foreground">{item.product?.name || `Product #${item.productId}`}</div>
                        <div className="text-muted-foreground">{item.quantity} x ${Number(item.unitPrice).toFixed(2)}</div>
                      </div>
